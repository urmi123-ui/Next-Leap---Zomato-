...
Role
LLMClient
Thin wrapper over Groq / Anthropic / Ollama / etc.
PromptTemplates
...
Implementations: GroqClient, AnthropicClient, OllamaClient for local dev.
...
### UI Styling & CSS overrides (Streamlit)
When applying a custom dark theme to a Streamlit application using HTML-injected CSS:
- **Selectbox Dropdowns**: Streamlit selectbox popover menus are rendered in portals outside the main app container. To prevent "white-on-white" text visibility bugs where dark-themed elements inherit default light theme backgrounds, style `div[data-baseweb="popover"]` and its list items explicitly (matching background `#161925` and text `#e2e8f0`).
- **Submit Buttons**: Ensure text color properties targeting Streamlit buttons (`div.stButton > button`) are also applied to child `<p>` and `<span>` tags, which otherwise inherit default body text settings and render as illegible grey text on top of active gradients.
